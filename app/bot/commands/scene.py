...
async def talk(
    ...
    try:
        progressed = await record_quest_progress(interaction.user.id, "talk", target=npc, game_minute=wt_now.total_minutes)
...
async def _resolve_scene_action(
    ...
    try:
        ...
        wt_now = await current_world_time()
        progressed = await record_quest_progress(interaction.user.id, "scene_action", amount=1, target=action_key, game_minute=wt_now.total_minutes)
        await announce_quest_progress(interaction, progressed)
    except Exception:
