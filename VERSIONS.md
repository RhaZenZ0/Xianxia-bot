...
the beginner path hands everybody, asked for it by name; it is open at Body Tempering now, and
drawn only in a capital's temple quarter, where the hall actually receives - anywhere else the
City page says where it is. The Realm Capitals role now counts a capital's gates, districts,
shops and auction halls as the capital, so arriving at the South Gate no longer takes the channel
away. And every inn's common room is somewhere its patrons can read it: a capital's inn keeps
its thread in the capital channel, and the other forty-four inns - whose threads sat in a
channel only somebody standing in the capital could see, so their card read "Common room:
#unknown" - hang theirs in their world's own feed.

**1.7.1** lets a market stall sell a trade's goods only for somebody who holds that trade's certificate, so a pill, a blade, a talisman or an array disk goes on a stall once its seller has passed an examination of the trade in this life, and lets the town buy every grade from a stall.

Any rank counts, because a rank rises on crafting alone and a cultivator can reach the second
without ever sitting the first; a pass in an earlier life does not, since samsara wipes the trade.
Raw materials - herbs, ore, cores, paper and ink - stay anybody's to sell, a trade that authors no
examination locks nothing, and listings already standing are left alone. The refusal names the
examination and the hall it is sat at, `/stall status` lists what in the bag waits on a
certificate, and the `/stall list` picker offers only what the stall will take. A shop-bought pill is
a recipe's output too, so an uncertified character cannot resell one at a stall; the shop counters
still buy it back.

The town buys every grade from a stall now. v1.7.0 let it buy only what some shop shelf sells, so
High and finer went to cultivators alone; a grade no shelf carries is priced off its Low item's
cheapest shelf at the grade's worth, one coin under - a High recovery pill at 43 or less where the
Low one sells for 11. Nothing sells the grade for that to loop through, and an ungraded item no
shelf sells is still never bought.

A bot without the Administrator permission can post in its own read-only channels again - `#updates`,
where the release notes go, among them - because Setup and Repair allow the bot before they deny
everybody else. Repair no longer undoes the gate that hides `#expeditions` and `#player-homes` from
people with no character, either. And the Discord playtest no longer fails a random button when the
bot's background tick happens to hold the engine at the moment it is pressed: the periodic workers run
once at boot and are stopped for the sweep.

The travel picker no longer offers a place the shop door will not open onto. From inside a shop it
lists the street first and then the city's other shops, which is all the door allows; it used to list
every gate, district and city the player knew, and the engine refused them. The same fix covers a
gate or shop of another city, the inside of an auction hall and the `/travel go` slash command. Every
one of the 104 shops with a door had the fault.

The engine playtest's sect road holds too. v1.7.0 had already moved its steps to where v1.3.1's
engine-side rules ask, and two halves were still missing: the sect discovery passed while recording
nothing, so it is held to having found the sect now, and the sponsor is moved to the foothills with
the audited GM lever and put back, because asking him there still passed or failed on the hour. No schema change.
...
NPCs holds the roster (now with the cast of every running world event), Population & Whereabouts
(who is where, who is away from home, missing, stalled at a ceiling or near the end of their years),
Marriage & Family, Society (feuds, friendships, masters and disciples, the people the world made for
itself), and Deeds & Fates, which shows a GM the hidden rows no player ever sees - an unwitnessed
crime, a grave robbed in the wilderness - because a GM is not a player. Sects holds Sect Politics,
Members & Lineage, Recruitment (every trial with the TN the engine rolled against, the standing
outsiders earned, the gates found, the hidden sect) and Holdings (manors and their projects, each
sect's real treasury, and the residences it granted). Both heads only read; no lever moved.

Three faults a read of the old pages found are fixed with them. The NPC roster's status filter had
never worked - it read `window.status` and sent "undefined" - so it now finds the missing, the dead
and the living; the Player Editor's sect card pre-filled a rank it never read and demoted a player
on an unchanged save; and the sect cards' prestige and treasury, which nothing writes, are replaced
by the items each sect really holds. A dig is refused in a shop and on an auction floor beside the
private places, shrines and open events it already refused; a waystation's yard, which carries its
keeper's stall, stays open ground. No schema change.
