...
bot control channel to the LAN. Both now default to `127.0.0.1`; `docker-compose.yml` sets `0.0.0.0`
explicitly on the two services whose neighbours need it, with the reason beside each line, and the
README carries a reverse-proxy recipe for LAN access with TLS.
