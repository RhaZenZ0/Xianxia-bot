...
func applyAuthoritativeQuery(databasePath, worldPath string, req ActionRequest) (ActionResponse, error) {
	...
	case "stall.board", "stall.status":
		...
		var qerr error
		if req.Operation == "stall.board" {
			result, qerr = stallBoardQuery(conn, catalog, req.ActorID)
		} else {
			result, qerr = stallStatusQuery(conn, catalog, req.ActorID)
		}
		if qerr != nil {
			return ActionResponse{}, qerr
		}
	...
	case "shop.here", "shop.browse":
		...
		var result map[string]any
		var qerr error
		if req.Operation == "shop.here" {
			result, qerr = shopHereQuery(conn, catalog, req.ActorID)
		} else {
			result, qerr = shopBrowseQuery(conn, catalog, req.ActorID)
		}
		if qerr != nil {
			return ActionResponse{}, qerr
		}
	...
	case "trade.status":
		...
		result, qerr := tradeStatusQuery(conn, catalog, req.ActorID)
		if qerr != nil {
			return ActionResponse{}, qerr
		}
		v, _ := eventledger.CurrentActorVersion(conn, req.ActorID)
