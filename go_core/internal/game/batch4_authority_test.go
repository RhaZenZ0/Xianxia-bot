...
CREATE TABLE characters(
    ...
    -- checkPlayerOldAgeDeathTx (lifespan.go) runs ahead of every authoritative
    -- mutation and reads these four columns directly off characters; matches
    -- the defaults setupCharacterCreationAuthorityDB already uses so a fresh
    -- fixture character here is never mistaken for one who died of old age
    -- (max game_minute exercised by this package's tests is in the low
    -- thousands, far short of 75 natural years = ~39.4M minutes).
...
CREATE TABLE character_qi_body(user_id INTEGER PRIMARY KEY,purity INTEGER NOT NULL DEFAULT 50,meridians_open INTEGER NOT NULL DEFAULT 12,meridians_damaged INTEGER NOT NULL DEFAULT 0,dantian_state TEXT NOT NULL DEFAULT 'intact',settled_game_minute INTEGER NOT NULL DEFAULT 0,qi_type TEXT NOT NULL DEFAULT 'spirit',corruption INTEGER NOT NULL DEFAULT 0,ghost_form INTEGER NOT NULL DEFAULT 0,created_at REAL NOT NULL DEFAULT 0,updated_at REAL NOT NULL DEFAULT 0);
...
CREATE TABLE world_crossings(location_key TEXT PRIMARY KEY,name TEXT NOT NULL DEFAULT '',from_world TEXT NOT NULL,to_world TEXT NOT NULL,destination_location TEXT NOT NULL,min_realm_index INTEGER NOT NULL DEFAULT 0,opened_realm_index INTEGER NOT NULL DEFAULT 0,cost INTEGER NOT NULL DEFAULT 0,opened_by_user_id INTEGER,opened_game_minute INTEGER NOT NULL DEFAULT 0,player_uses INTEGER NOT NULL DEFAULT 0,npc_uses INTEGER NOT NULL DEFAULT 0,created_at REAL NOT NULL DEFAULT 0,FOREIGN KEY(opened_by_user_id) REFERENCES characters(user_id) ON DELETE SET NULL);
...
func TestBatch4TribulationPrepareAndAttemptPersistCanonicalOutcome(t *testing.T) {
	...
	// Fixture seeds fate at 1 (below the addFateGo cap of 9), and clearing this
	// tribulation now calls addFateGo exactly once - progression_actions.go's
	// previous duplicate call (V019's "Tribulation double fate grant" fix) was
	// removed, so a single successful clear grants +1, landing on 2, not the
	// pre-fix double-grant total of 3.
