# The v1.0.0 release candidates

The per-release changelog entries as written, `v1.0.0-rc.1` to `v1.0.0-rc.59` — **54 entries across
59 candidates**, because rc.11, rc.12, rc.29, rc.30 and rc.31 never got one of their own (rc.11 and
rc.12 were tagged on the same commit). Moved here at
**v1.0.0** when `VERSIONS.md` was trimmed the way it was at rc.1 — the same move at the next
boundary, and for the same reason: the changelog an operator opens should be the release they are
running, not fifty-nine candidates of how it got there.

`VERSIONS.md` keeps the `**1.0.0**` entry, a paragraph per minor below it, the release status and
the whole schema history. Nothing here describes the current release.

One consequence worth stating, because it is a behaviour and not only a filing decision: the bot
reads `VERSIONS.md` live to post release notes into `#updates`, matching the entry for the version
it is running. A bot still running a release candidate now finds no entry for itself, logs that,
and posts nothing — it does not advance its marker, so the notes it eventually posts are 1.0.0's.
