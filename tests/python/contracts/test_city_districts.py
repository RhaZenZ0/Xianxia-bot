...
class TheEngineOwnsTheWalk(unittest.TestCase):
    def test_arrival_is_at_the_facing_gate_and_the_parts_are_known(self):
        ...
        for needle in ("func cityOf(", "func cityPartsOf(", "func gateFacing(", 'gateFacing(catalog, p.Destination, roadFacingNeighbour(catalog, p.Destination, route[len(route)-2]))', '"arrived_at":', '"left_by_gate":', '"city_parts":', "canonicalRoadRouteRiding(catalog, originCity, p.Destination, c.accessRealmIndex(), riding, mount)"):
