...
from app.birthfamily import FAMILY_ARCHETYPES
from app.creation_ui import CULTIVATION_STYLE_PROFILES
...
class GUIIntegrityTests(unittest.TestCase):
    def test_character_sheet_never_declares_more_than_discord_field_limit(self):
        ...
        start = source.index('@registered_root_command(name="sheet"')
        end = source.index('aptitude_group =', start)
    ...
    def test_admin_panel_has_seven_sections_and_all_actions_fit_selects(self):
        source = BOT.read_text(encoding="utf-8")
        start = source.index("_ADMIN_HUB_DEFINITION = HubDefinition(")
        end = source.index("@registered_root_command(", start)
    ...
    def test_known_invalid_refresh_glyph_is_absent(self):
        combined = BOT.read_text(encoding="utf-8") + HUBS.read_text(encoding="utf-8")
