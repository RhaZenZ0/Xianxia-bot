...
/* A retreat's deadline is real seconds, not game minutes (schema 57,
   v1.0.0-rc.56): a GM changing the world's time scale must not re-size a
   retreat already under way, so `ends_game_minute` is a projection that
   follows the real deadline and this is the deadline. */
function fmtReal(ts){ts=Number(ts||0); if(!ts) return '—'; const left=Math.round((ts*1000-Date.now())/60000); if(left<=0) return 'due'; return left>=60?`${Math.floor(left/60)}h ${left%60}m`:`${left}m`}
...
const VIEWS={
  ...
  npcs:{t:'NPC Roster',g:'NPCs',b:'Who is alive, where they are, and what they are trying to do, plus the cast of every running world event. A card opens the full definition.'},
  npc_population:{t:'Population & Whereabouts',g:'NPCs',b:'How many people the world holds and where they stand: who is away from home, who is missing, who has stopped at a ceiling, and who is near the end of their years.'},
  npc_families:{t:'Marriage & Family',g:'NPCs',b:'Married couples, courtships, weddings, the children the simulation has had, the widowed, and the relatives each starter household carries.'},
  npc_society:{t:'Society',g:'NPCs',b:'Feuds and friendships between NPCs, masters and their disciples, the people the world made for itself, and who stands in which sect.'},
  npc_deeds:{t:'Deeds & Fates',g:'NPCs',b:'Everything NPCs did on their own and how lives ended. Hidden rows are shown: an unwitnessed crime never reaches a player, but a GM is not a player.'},
  sects:{t:'Sect Politics',g:'Sects',b:'Influence, cohesion, treasuries and the treaties between the simulated sects. Internal factions have their own agendas.'},
  sect_members:{t:'Members & Lineage',g:'Sects',b:'Every player in a sect with their rank and contribution, who taught whom, and the disciple requests still waiting.'},
  sect_recruitment:{t:'Recruitment',g:'Sects',b:'Every trial and recommendation with the TN the engine rolled against, the standing players earned as outsiders, the gates they found, and the hidden sect.'},
  sect_holdings:{t:'Holdings',g:'Sects',b:'What each sect owns: its manor and the projects that raised it, the items in its treasury, and the residences it granted.'},
  ...
  admin:{t:'Admin Console',g:'Admin',b:'Actions that change the world or the server. Every one is applied by the engine and written to admin_audit_log with your name on it. One character\'s levers live on the Player Editor.'},
  player_editor:{t:'Player Editor',g:'Admin',b:'One character at a time: the sheet as the engine holds it, and every per-player lever pre-filled from the row it writes. Every change is applied by the engine and audited.'},
};
...
async function loadNPCs(){const d=await api('/api/npcs?limit=300&status=alive');app.innerHTML=`<h2>Active NPCs</h2>${filters(`<input id="q" placeholder="Name, job, goal"><select id="loc"><option value="">All locations</option>${d.locations.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="fac"><option value="">All factions</option>${d.factions.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="npcStatus"><option>alive</option><option>missing</option><option>dead</option><option>all</option></select>`)}<div id="npcRows">${npcCards(d.rows)}</div><h2>Event Casts</h2><p class="muted">The temporary people a running world event brings: the militia captain, the visiting elder, the auctioneer. They are not in the permanent catalogue and stop answering when their event closes.</p>${table([['Event',r=>`<b>${esc(r.event_title||r.event_key)}</b><div class="muted">${esc(r.event_type||'')} · ${esc(r.event_location||'')}</div>`],['Name','name'],['Title / role',r=>esc([r.title,r.role].filter(Boolean).join(' · ')||'—')],['Location','location'],['Speaks for',r=>esc(r.sect_name||'—')],['Can recommend',r=>Number(r.can_recommend)?pill('yes','good'):'—']],d.event_casts||[],{empty:'No running world event has a cast.'})}`;bindNpc();document.getElementById('applyFilters').onclick=async()=>{const p=new URLSearchParams({limit:'300',q:document.getElementById('q').value,location:document.getElementById('loc').value,faction:document.getElementById('fac').value,status:document.getElementById('npcStatus').value});const x=await api('/api/npcs?'+p);document.getElementById('npcRows').innerHTML=npcCards(x.rows);bindNpc()}}
function npcCards(rows){return `<div class="npc-grid">${rows.map(r=>`<div class="card npc-card" data-npc="${esc(r.npc_name)}"><h3><span class="status-dot ${r.status==='dead'?'dead':''}"></span>${esc(r.npc_name)}${r.status==='missing'?' '+pill(`Missing ${n(r.days_missing)}d`,'bad'):''}</h3><div class="tagline">${pill(`Realm ${r.realm_index}/${r.phase}`,'blue')}${pill(r.sect_rank||r.faction,'warn')}${r.injury_severity?pill(`Injury ${r.injury_severity}`,'bad'):''}</div><div class="row"><span>Location</span><b>${esc(r.current_location)}</b></div><div class="row"><span>Activity</span><b>${esc(r.activity)}</b></div><div class="row"><span>Mood</span><b>${esc(r.mood||'—')}</b></div><div class="row"><span>Goal</span><b>${esc(r.current_goal||'—')}</b></div><div class="row"><span>Family</span><b>${esc(r.relationship_status||'single')}${r.spouse_name?` · ${esc(r.spouse_name)}`:''}</b></div></div>`).join('')}</div>`}
function bindNpc(){document.querySelectorAll('.npc-card,.npc-link').forEach(el=>el.onclick=()=>showNpc(el.dataset.npc))}
async function showNpc(name){const d=await api('/api/npc?name='+encodeURIComponent(name));const r=d.npc||{};if(!r.npc_name){drawerBody.innerHTML=`<h2>${esc(name)}</h2><div class="empty">No simulation record for this name yet.<span class="why">A child who has not come of age, a household relative, or a running event's cast has no row in the simulation until the world gives them one.</span></div>`;drawer.classList.remove('hidden');return}drawerBody.innerHTML=`<h2>${esc(r.npc_name)}</h2><div class="tagline">${pill(r.status)} ${pill(`Realm ${r.realm_index}/${r.phase}`,'blue')} ${pill(r.sect_rank,'warn')} ${r.injury_severity?pill(`Injury ${r.injury_severity}`,'bad'):''}</div><div class="section"><h3>Current life state</h3>${[['Location',r.current_location],['Faction',r.faction],['Profession',r.profession],['Activity',r.activity],['Mood',r.mood],['Goal',r.current_goal],['Goal progress',`${r.goal_progress||0}%`],['Health',r.health],['Injury',r.injury||'None'],['Spouse',r.spouse_name||'None'],['Children',r.children_count||0]].map(x=>`<div class="row"><span>${x[0]}</span><b>${esc(x[1])}</b></div>`).join('')}</div><div class="section"><h3>Social relations</h3>${table([['NPC A','npc_a'],['NPC B','npc_b'],['Type','relation_type'],['Affinity','affinity'],['Trust','trust'],['Grudge','grudge']],d.social)}</div><div class="section"><h3>Master / disciple</h3>${table([['Master','master_name'],['Disciple','disciple_name'],['Status','status'],['Reason','reason']],d.disciples)}</div><div class="section"><h3>Descendants</h3>${table([['Child','child_name'],['Parent A','parent_a'],['Parent B','parent_b'],['Root','spiritual_root'],['Status','status']],d.descendants)}</div><div class="section"><h3>Canonical history</h3>${timeline(d.history)}</div><details><summary>GM-only canonical NPC definition</summary><div class="code">${esc(JSON.stringify(d.gm_definition,null,2))}</div></details>`;drawer.classList.remove('hidden')}
/* The NPCs head (v1.6.0). Every value is read, none decided here. */
const realmOf=r=>`Realm ${esc(r.realm_index??'—')}/${esc(r.phase??'—')}`;
const npcLink=name=>name?`<span class="npc-link" data-npc="${esc(name)}">${esc(name)}</span>`:'—';
function groupTable(title,rows,label){return panel(title,table([[label,r=>esc(r.key===null||r.key===undefined||r.key===''?'—':String(r.key))],['NPCs',r=>n(r.n)]],rows,{empty:'Nobody.'}),{count:`${n(rows.reduce((a,r)=>a+Number(r.n||0),0))}`})}
async function loadNpcPopulation(){const d=await api('/api/npc_population');const s=d.summary||{};
 app.innerHTML=`<div class="metrics">
  ${metric('Alive',n(s.alive))}
  ${metric('Missing',n(s.missing),'',Number(s.missing)?'warn':'')}
  ${metric('Dead',n(s.dead))}
  ${metric('Married',n(s.married))}
  ${metric('Injured',n(s.injured),'',Number(s.injured)?'warn':'')}
  ${metric('Average age',s.average_age===null||s.average_age===undefined?'—':esc(s.average_age))}
  ${metric('Near the end',n(s.near_end_of_lifespan),'five years or fewer',Number(s.near_end_of_lifespan)?'warn':'')}
 </div>
 <h2>Groupings</h2><div class="grid2">${groupTable('By world',d.by_world||[],'World')}${groupTable('By faction',d.by_faction||[],'Faction')}${groupTable('By profession',d.by_profession||[],'Profession')}${groupTable('By realm',d.by_realm||[],'Realm')}</div>
 <h2>Who is where</h2>${table([['Location','location'],['World','world_name'],['NPCs',r=>n(r.npcs)],['Region population',r=>r.population===null||r.population===undefined?'—':n(r.population)],['Prosperity',r=>esc(r.prosperity??'—')],['Security',r=>esc(r.security??'—')],['Unrest',r=>esc(r.unrest??'—')]],d.whereabouts||[],{empty:'Nobody stands anywhere yet.'})}
 <h2>Away from home</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Home','home_location'],['Now at','current_location'],['Status',r=>pill(r.status,r.status==='missing'?'bad':'good')],['Activity','activity'],['Realm',realmOf]],d.away_from_home||[],{empty:'Everybody is at home.'})}
 <h2>Stalled</h2><p class="muted">Two ceilings the simulation writes into an NPC's activity: an ascension gate nobody may cross until a player tears a seam open out of that world, and the talent a person was born with.</p>${table([['NPC',r=>npcLink(r.npc_name)],['Ceiling',r=>pill(r.activity,'warn')],['Realm',realmOf],['Profession','profession'],['Faction','faction'],['Location','current_location']],d.stalled||[],{empty:'Nobody is stalled.'})}
 <h2>Missing</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Days missing',r=>pill(n(r.days_missing),'bad')],['Home','home_location'],['Last known','current_location'],['Since',r=>fmtGM(r.missing_since_game_minute)],['Realm',realmOf]],d.missing||[],{empty:'Nobody is missing.'})}
 <h2>Near the end</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Age',r=>esc(r.age_years)],['Lifespan',r=>esc(r.natural_lifespan_years)],['Years left',r=>pill(r.remaining_years,Number(r.remaining_years)<=0?'bad':'warn')],['Location','location']],d.near_end_of_lifespan||[],{empty:'Nobody is near the end of their years.'})}`;bindNpc()}
async function loadNpcFamilies(){const d=await api('/api/npc_families');
 app.innerHTML=`<h2>Married couples</h2>${table([['Partner',r=>npcLink(r.npc_a)],['Partner',r=>npcLink(r.npc_b)],['Children',r=>n(r.children_count)],['First stands at',r=>`${esc(r.location_a||'—')} · ${esc(r.realm_a??'—')}/${esc(r.phase_a??'—')}`],['Second stands at',r=>`${esc(r.location_b||'—')} · ${esc(r.realm_b??'—')}/${esc(r.phase_b??'—')}`],['Married since',r=>r.married_since?fmtGM(r.married_since):'—']],d.couples||[],{empty:'Nobody is married.'})}
 <h2>Courtships</h2>${table([['NPC',r=>npcLink(r.npc_a)],['NPC',r=>npcLink(r.npc_b)],['Affinity','affinity'],['Trust','trust'],['Started',r=>fmtGM(r.started_game_minute)],['Last together',r=>fmtGM(r.last_interaction_game_minute)],['Where',r=>`${esc(r.location_a||'—')} / ${esc(r.location_b||'—')}`]],d.courtships||[],{empty:'Nobody is courting.'})}
 <h2>Weddings</h2>${timeline(d.weddings||[])}
 <h2>Children</h2>${table([['Child',r=>npcLink(r.child_name)],['Parents',r=>`${npcLink(r.parent_a)} &amp; ${npcLink(r.parent_b)}`],['Age',r=>esc(r.age_years??'—')],['Gender','gender'],['Root','spiritual_root'],['Realm',realmOf],['Status','status'],['Grown',r=>Number(r.generated_as_npc)?pill('an NPC now','good'):pill('a child','blue')],['Came of age',r=>r.came_of_age_game_minute?fmtGM(r.came_of_age_game_minute):'—']],d.children||[],{empty:'No children have been born.'})}
 <h2>Widowed</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Children',r=>n(r.children_count)],['Faction','faction'],['Location','current_location'],['Status','status']],d.widows||[],{empty:'Nobody has been widowed.'})}
 <h2>Household relatives</h2>${table([['Family','family_name'],['Home','family_location'],['Name','name'],['Relation','relation'],['Gender','gender'],['Status','status'],['Realm',realmOf]],d.relatives||[],{empty:'No starter household carries relatives yet.'})}`;bindNpc()}
async function loadNpcSociety(){const d=await api('/api/npc_society');const origins=(d.registry_by_origin||[]).map(r=>pill(`${r.origin} ${r.n}`,'purple')).join(' ');
 app.innerHTML=`<h2>Feuds and grudges</h2>${table([['NPC',r=>npcLink(r.npc_a)],['NPC',r=>npcLink(r.npc_b)],['Type',r=>pill(r.relation_type,'bad')],['Grudge','grudge'],['Affinity','affinity'],['Trust','trust'],['Last clash',r=>fmtGM(r.last_interaction_game_minute)]],d.feuds||[],{empty:'Nobody holds a grudge.'})}
 <h2>Friendships</h2>${table([['NPC',r=>npcLink(r.npc_a)],['NPC',r=>npcLink(r.npc_b)],['Type','relation_type'],['Affinity','affinity'],['Trust','trust'],['Since',r=>fmtGM(r.started_game_minute)]],d.friendships||[],{empty:'No friendships recorded.'})}
 <h2>Masters and disciples</h2>${table([['Master',r=>npcLink(r.master_name)],['Disciple',r=>npcLink(r.disciple_name)],['Status',r=>pill(r.status,r.status==='active'?'good':'')],['Master’s faction','master_faction'],['Since',r=>fmtGM(r.started_game_minute)],['Ended',r=>r.ended_game_minute?fmtGM(r.ended_game_minute):'—'],['Reason',r=>esc(r.reason||'—')]],d.disciple_bonds||[],{empty:'No NPC has taken a disciple.'})}
 <h2>The world's own people</h2><p class="muted">People the world made after the content file: children who came of age, household relatives, and anyone a GM added. ${origins}</p>${table([['Name',r=>npcLink(r.name)],['Origin',r=>pill(r.origin,'purple')],['Role','role'],['Realm','realm'],['Location','location'],['Sect',r=>esc(r.sect_affiliation||'—')],['Source','source_key'],['Since',r=>fmtGM(r.created_game_minute)]],d.registry||[],{empty:'The world has made nobody yet.'})}
 <h2>Sect membership</h2><div class="grid2">${panel('Living NPCs by faction',table([['Faction','faction'],['NPCs',r=>n(r.n)]],d.sect_members||[],{empty:'Nobody.'}))}${panel('Ranks inside each faction',table([['Faction','faction'],['Rank',r=>esc(r.sect_rank||'—')],['NPCs',r=>n(r.n)]],d.sect_ranks||[],{empty:'No NPC has a life record yet.'}))}</div>`;bindNpc()}
async function loadNpcDeeds(){const d=await api('/api/npc_deeds?limit=150');
 app.innerHTML=`<h2>Feed</h2>${filters(`<input id="deedQ" placeholder="Name, title, summary"><select id="deedType"><option value="">All deed types</option>${(d.event_types||[]).map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="deedVis"><option value="">All visibility</option><option>public</option><option>participant</option><option>faction</option><option>hidden</option></select>`)}<div id="deedRows">${timeline(d.rows||[])}</div>
 <h2>Deaths</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Died',r=>fmtGM(r.death_game_minute)],['Cause',r=>esc(r.cause_of_death||'—')],['Home','home_location'],['Last at','current_location'],['Faction','faction'],['Realm',realmOf]],d.deaths||[],{empty:'Nobody has died yet.'})}${(d.lost||[]).length?`<h3>Lost to the wilderness</h3>${timeline(d.lost)}`:''}
 <h2>Missing</h2>${table([['NPC',r=>npcLink(r.npc_name)],['Days missing',r=>pill(n(r.days_missing),'bad')],['Home','home_location'],['Last known','current_location'],['Since',r=>fmtGM(r.missing_since_game_minute)]],d.missing||[],{empty:'Nobody is missing.'})}
 <h2>Graves</h2><p class="muted">Schema 48: people the world lost and never found. A grave holds what they were carrying, and the first searcher to reach it takes it &mdash; which may be the world&rsquo;s own grave-robbers, who get their chance once the searcher&rsquo;s window has passed. A robbery is <em>hidden</em>, so no player is told; you are not a player.</p>${table([['NPC','npc_name'],['Found at','location'],['Home','home_location'],['Days missing','days_missing'],['Keepsake','keepsake_item'],['Stones','keepsake_stones'],['Reached by',r=>r.claimed_by_name||(r.claimed_by_user_id?id(r.claimed_by_user_id):(r.robbed_by?esc(r.robbed_by)+' (robbed)':(r.claimed_game_minute?'emptied':'—')))]],d.graves||[])}
 <h2>Breakthroughs</h2>${timeline(d.breakthroughs||[])}`;
 bindNpc();
 document.getElementById('applyFilters').onclick=async()=>{const p=new URLSearchParams({limit:'300',q:document.getElementById('deedQ').value,event_type:document.getElementById('deedType').value,visibility:document.getElementById('deedVis').value});const x=await api('/api/npc_deeds?'+p);document.getElementById('deedRows').innerHTML=timeline(x.rows||[])}}
async function showPlayer(uid){const d=await api('/api/player?user_id='+encodeURIComponent(uid));const r=d.player||{};if(!r.user_id){drawerBody.innerHTML='<div class="empty">Player not found.</div>';drawer.classList.remove('hidden');return}const scene=d.scene||{};drawerBody.innerHTML=`<h2>${esc(r.name)}</h2><div class="tagline">${pill(r.life_status,r.life_status==='alive'?'good':'bad')} ${pill(`Realm ${r.realm_index}/${r.phase}`,'blue')} ${pill(r.location)} <button class="btn sm primary" data-edit-player="${esc(r.user_id)}">✎ Open in Player Editor</button></div><div class="section"><h3>Character sheet</h3>${[['Discord',r.discord_name],['Origin',r.origin],['Path',r.path],['Spiritual Root',r.spiritual_root],['Gender',r.gender],['Karma',r.karma_score],['Cultivation Realm',`${r.realm_index}/${r.phase}`],['Body Realm',`${r.body_realm_index}/${r.body_phase}`],['Vitality',`${r.vitality}/${r.vitality_max}`],['Qi',`${r.qi}/${r.qi_max}`],['Spirit Stones',n(r.spirit_stones)],['Sect',r.sect_name?`${r.sect_name} · ${r.rank_name}`:'Independent'],['Updated',r.updated_at?new Date(Number(r.updated_at)*1000).toLocaleString():'—']].map(x=>`<div class="row"><span>${x[0]}</span><b>${esc(x[1])}</b></div>`).join('')}</div><div class="section"><h3>Inventory</h3>${table([['Item','item_id'],['Qty','quantity']],d.inventory||[])}</div><div class="section"><h3>Cooldowns</h3>${table([['Action','action'],['Available at',r=>new Date(Number(r.available_at)*1000).toLocaleString()]],d.cooldowns||[])}</div><div class="section"><h3>Conditions</h3>${table([['Condition','name'],['Category','category'],['Severity','severity'],['',c=>`<button class="btn" data-clear-condition="${c.condition_id}">Clear</button>`]],d.conditions||[])}</div><div class="section"><h3>Scene state</h3>${scene.scene_type?[['Type',scene.scene_type],['Key',scene.scene_key],['Label',scene.scene_label],['Physical Location',scene.physical_location]].map(x=>`<div class="row"><span>${x[0]}</span><b>${esc(x[1])}</b></div>`).join(''):'<div class="empty">No scene state recorded.</div>'}</div>`;drawer.classList.remove('hidden');document.querySelectorAll('[data-edit-player]').forEach(el=>el.onclick=()=>openPlayerEditor(el.dataset.editPlayer));document.querySelectorAll('[data-clear-condition]').forEach(el=>el.onclick=async()=>{if(!confirm('Clear this condition?'))return;try{await adminPost('player.clear_condition',{user_id:uid,condition_id:Number(el.dataset.clearCondition),reason:'GM cleared condition'});await showPlayer(uid)}catch(e){alert(e.message)}})}
...
async function loadSects(){const d=await api('/api/sects');app.innerHTML=`<h2>Sect Politics</h2><div class="sect-grid">${d.sects.map(s=>`<div class="card sect-card"><h3>${esc(s.sect_name)}</h3><div class="tagline">${pill(s.alignment)} ${pill(s.specialty,'blue')}</div><div class="row"><span>Influence</span><b>${s.influence}</b></div><div class="bars"><span style="width:${Math.max(0,Math.min(100,s.influence))}%"></span></div><div class="row"><span>Cohesion / Resources</span><b>${s.cohesion} / ${s.resources}</b></div><div class="row"><span>Recruitment / Doctrine</span><b>${s.recruitment_pressure} / ${s.doctrine_pressure}</b></div><div class="row"><span>Policy</span><b>${esc(s.leader_policy)}</b></div><div class="row"><span>Players / NPCs</span><b>${s.player_members} / ${s.npc_members}</b></div><div class="row"><span>Treasury</span><b>${n(s.treasury_units)} units · ${n(s.treasury_items)} kinds</b></div></div>`).join('')}</div><h2>Internal Factions</h2>${table([['Sect','sect_name'],['Faction','faction_name'],['Agenda','agenda'],['Power','power'],['Loyalty','loyalty']],d.factions)}<h2>Inter-Sect Relations</h2>${table([['Sect A','sect_a'],['Sect B','sect_b'],['Score','relation_score'],['Type','relation_type'],['Treaty','treaty_status']],d.relations)}<h2>Recent Political Developments</h2>${table([['Sect','sect_name'],['Event','event_text'],['Severity','severity'],['Time',r=>fmtGM(r.game_minute)]],d.events)}`}
/* The Sects head (v1.6.0). A player row opens the character sheet. */
const playerLink=(uid,name)=>uid===null||uid===undefined||uid===''?esc(name||'—'):`<span class="npc-link" data-player="${esc(uid)}">${esc(name||uid)}</span>`;
function bindPlayers(){document.querySelectorAll('[data-player]').forEach(el=>el.onclick=()=>showPlayer(el.dataset.player))}
const fmtTs=ts=>Number(ts||0)?new Date(Number(ts)*1000).toLocaleString():'—';
async function loadSectMembers(){const d=await api('/api/sect_members');
 app.innerHTML=`<h2>By sect</h2>${table([['Sect','sect_name'],['Members',r=>n(r.members)],['Average rank',r=>esc(r.average_rank??'—')],['Highest rank',r=>esc(r.highest_rank??'—')],['Contribution',r=>n(r.contribution)]],d.by_sect||[],{empty:'No player is in a sect.'})}
 <h2>Members</h2>${table([['Player',r=>playerLink(r.user_id,r.name)],['Discord','discord_name'],['Life',r=>pill(r.life_status,r.life_status==='alive'?'good':'bad')],['Sect','sect_name'],['Rank',r=>`${esc(r.rank_name)} (${esc(r.rank_level)})`],['Contribution',r=>n(r.contribution_points)],['Influence',r=>n(r.influence)],['Realm',r=>`${esc(r.realm_index)}/${esc(r.phase)}`],['Location','location'],['Joined',r=>fmtTs(r.joined_at)]],d.members||[],{empty:'No player is in a sect.'})}
 <h2>Masters and disciples</h2>${table([['Master',r=>playerLink(r.master_user_id,r.master_name)],['Disciple',r=>playerLink(r.disciple_user_id,r.disciple_name)],['Sect',r=>esc(r.sect_name||'—')],['Attention','attention'],['Since',r=>fmtTs(r.accepted_at)]],d.lineage||[],{empty:'No player has taken a disciple.'})}
 <h2>Disciple requests</h2>${table([['Would-be disciple',r=>playerLink(r.disciple_user_id,r.disciple_name)],['Asked of',r=>playerLink(r.master_user_id,r.master_name)],['Status',r=>pill(r.status,r.status==='pending'?'warn':'')],['Asked',r=>fmtTs(r.created_at)],['Settled',r=>fmtTs(r.resolved_at)]],d.requests||[],{empty:'No disciple requests.'})}`;bindPlayers()}
async function loadSectRecruitment(){const d=await api('/api/sect_recruitment');
 app.innerHTML=`<h2>Trials and recommendations</h2><p class="muted">Every attempt to get in, with the score the roll reached and the TN it was rolled against, exactly as the engine stored them when it decided.</p>${table([['When',r=>fmtGM(r.game_minute)],['Player',r=>playerLink(r.user_id,r.player_name)],['Sect','sect_name'],['Kind',r=>pill(r.attempt_type,'blue')],['Result',r=>pill(r.result,/pass|success|accept/i.test(r.result||'')?'good':'bad')],['Score vs TN',r=>`${esc(r.score)} vs ${esc(r.target)}`],['Recommendation bonus',r=>esc(r.recommendation_bonus||'—')],['Sponsor',r=>esc(r.npc_name||'—')],['Where','location'],['Notes',r=>`<small>${esc(jsonText(r.details_json))}</small>`]],d.attempts||[],{empty:'Nobody has tried to join a sect yet.'})}
 <h2>Recommendations held</h2>${table([['Player',r=>playerLink(r.user_id,r.player_name)],['From','npc_name'],['Sect','sect_name'],['Bonus','bonus'],['Status',r=>pill(r.status,r.status==='active'?'good':'')],['Given',r=>fmtGM(r.issued_game_minute)],['Used',r=>r.used_game_minute?fmtGM(r.used_game_minute):'—']],d.recommendations||[],{empty:'No recommendations have been given.'})}
 <h2>Standing with sects</h2><p class="muted">Outsider standing from a sect's entry-level commissions, and the +5 a passed trial gives, both kept under the sect's own name.</p>${table([['Player',r=>playerLink(r.user_id,r.player_name)],['Sect','sect_name'],['Standing','score'],['Last reason',r=>esc(r.last_reason||'—')],['Updated',r=>fmtTs(r.updated_at)]],d.standing||[],{empty:'Nobody has standing with a sect yet.'})}
 <h2>Gates discovered</h2>${table([['Player',r=>playerLink(r.user_id,r.player_name)],['Sect','sect_name'],['How',r=>pill(r.discovery_kind,'purple')],['Source','source_key'],['When',r=>fmtGM(r.discovered_game_minute)]],d.discoveries||[],{empty:'No sect gate has been discovered.'})}
 <h2>Hidden sect</h2><p class="muted">Read directly from its membership rows. The player's own "status" door can settle a hostile membership, so this page never asks through it.</p>${table([['Player',r=>playerLink(r.user_id,r.player_name)],['Sect','sect_name'],['Rank','rank_name'],['Branch','branch_name'],['Standing','standing'],['Status',r=>pill(r.status,r.status==='active'?'good':'bad')],['Karma','karma_score'],['Joined',r=>fmtGM(r.joined_game_minute)]],d.hidden||[],{empty:'Nobody has been initiated.'})}`;bindPlayers()}
async function loadSectHoldings(){const d=await api('/api/sect_holdings');
 app.innerHTML=`<h2>Manors</h2>${table([['Sect','sect_name'],['Manor','name'],['Stands at','base_location'],['Qi array','qi_array_level'],['Alchemy hall','alchemy_hall_level'],['Forge pavilion','forge_pavilion_level'],['Defence array','defense_array_level'],['Founded by',r=>playerLink(r.founded_by_user_id,r.founder_name)],['Founded',r=>fmtGM(r.created_game_minute)]],d.manors||[],{empty:'No sect has founded a manor.'})}
 <h2>Manor projects</h2>${table([['When',r=>fmtGM(r.game_minute)],['Sect','sect_name'],['By',r=>playerLink(r.user_id,r.player_name)],['Project','project_type'],['Facility',r=>esc(r.facility_key||'—')],['Level',r=>`${esc(r.from_level)} → ${esc(r.to_level)}`],['Cost',r=>`<small>${esc(jsonText(r.cost_json))}</small>`]],d.projects||[],{empty:'No manor project has been raised.'})}
 <h2>Treasuries</h2>${table([['Sect','sect_name'],['Item',r=>`${esc(r.item_name)}<div class="muted">${esc(r.item_id)}</div>`],['Quantity',r=>n(r.quantity)]],d.treasury||[],{empty:'Every sect treasury is empty.'})}
 <h2>Residences</h2>${table([['Owner',r=>playerLink(r.user_id,r.owner_name)],['Sect','sect_name'],['Name','name'],['Base','base_location'],['Cultivation','cultivation_level'],['Alchemy','alchemy_level'],['Forge','forge_level'],['Formation','formation_level'],['Storage','storage_level'],['Herb garden','herb_garden_level']],d.abodes||[],{empty:'No sect has granted a residence.'})}`;bindPlayers()}
...
// table() escapes a string accessor but inserts a FUNCTION accessor raw, so a
// cell can return markup - which makes escaping the function's own job. Every
// other helper used that way does it (jsonText is wrapped in esc() at all five
// of its call sites); this one did not. Its ids come from a trade offer, and
// the engine's cleanTradeItems rejects any id that is not in the catalogue, so
// nothing hostile reaches it today - but that is a validation two layers away
// in another language standing in for an escape that belongs here.
function tradeSide(json,stones){let items={};try{items=JSON.parse(json||'{}')||{}}catch{}const parts=Object.entries(items).map(([k,v])=>`${esc(k)} ×${esc(v)}`);if(Number(stones||0))parts.push(`${esc(stones)} stones`);return parts.length?parts.join(', '):'nothing'}
...
async function loadDiscordSetup(){
 ...
 const guild=d.guild||{}, bot=d.bot||{}, perms=d.permissions||[], base=d.base_channels||[], realms=d.realm_hubs||[], feeds=d.world_event_feeds||[], markets=d.stall_markets||[], halls=d.auction_halls||[], channels=d.text_channels||[];
 ...
 <div class="cards">
  ...
  <div class="card"><small>World Events</small><div class="metric">${n(d.world_events_ready)}/${n(d.world_events_total)}</div></div>
 ...
 <div class="grid2">
  ...
  <section><h2>World Events</h2><p class="muted">One news channel per world (v1.0.0-rc.52), in their own <b>\u{1F320} World Events</b> category. A located event \u2014 a disaster, an invasion, a disappearance, a secret realm coming open \u2014 is announced in the channel of the world it happened in, with a link to its scene thread; <b>#world-events</b> stays as the global feed for the weekend gift, a world reset and anything else that has no one world. Each is visible to cultivators who have reached that world, the same access role that gates the auction floors. Setup/Repair creates them and moves existing ones into the category.</p>${table([['World','world'],['State',r=>pill(r.ready?'ready':'missing',r.ready?'good':'warn')],['Channel',r=>r.channel_name?`#${esc(r.channel_name)}`:`\u2014 (expects #${esc(r.expected_name||'')})`],['Access role',r=>esc(r.role_name||'\u2014')],['Visibility',r=>r.channel_name?pill(r.hidden?'that world only':'VISIBLE TO ALL',r.hidden?'good':'bad'):'\u2014']],feeds)}</section>
  <section><h2>Market Stalls</h2><p class="muted">One read-only market channel per world (v1.7.0), in their own <b>\u{1F9FA} Market Stalls</b> category. The bot keeps one card per open stall in the world the stall stands in, edited as goods are listed, bought and withdrawn and removed when the stall closes; players buy through <b>/economy \u2192 Market Stalls</b>. Each is visible to cultivators who have reached that world. Setup/Repair creates them, moves existing ones into the category and makes them read-only.</p>${table([['World','world'],['State',r=>pill(r.ready?'ready':'missing',r.ready?'good':'warn')],['Channel',r=>r.channel_name?`#${esc(r.channel_name)}`:`\u2014 (expects #${esc(r.expected_name||'')})`],['Access role',r=>esc(r.role_name||'\u2014')],['Read-only',r=>r.channel_name?pill(r.read_only?'bot only':'PLAYERS CAN POST',r.read_only?'good':'bad'):'\u2014']],markets)}</section>
  <section><h2>Auction Houses</h2><p class="muted">Every city has an auction house (v0.33.1). A capital's grand house has a live channel of its own; the local floors of a world - smaller, six lots at a time, none longer than six hours - share one channel per world. A lot is posted the moment it is listed, its card follows every bid, and it is struck when the simulation tick settles it. Visible to cultivators who can reach that world. Setup/Repair creates the channels, in their own <b>\u{1F3EE} Auction Houses</b> category (v1.0.0-rc.51 \u2014 they used to share the capitals'). Run Repair after upgrading to move existing channels into it.</p>${table([['House','name'],['Size',r=>pill(r.size,r.size==='grand'?'purple':'')],['World','world'],['Entrance','entrance'],['State',r=>pill(r.ready?'ready':'missing',r.ready?'good':'warn')],['Channel',r=>r.channel_name?`#${esc(r.channel_name)}`:'—'],['Open lots',r=>n(r.open_lots)]],halls)}</section>
 </div>
 <h2>Use Existing Channels</h2><section class="card control"><p>Choose existing text channels instead of the recommended generated layout. Blank selections preserve the current binding.</p><div class="grid2">${bind('bindAnnouncements','World events / announcements','world-events')}${bind('bindScenes','Event scenes (retired)','event-scenes')}${bind('bindHomes','Player homes','player-homes')}${bind('bindLogs','Bot logs','bot-logs')}${bind('bindBegin','Begin here','begin-here')}${bind('bindInfo','Xianxia info','xianxia-info')}${bind('bindExploration','Expeditions','expeditions')}${bind('bindPlaytest','Playtest board','playtest')}${bind('bindUpdates','Release updates','updates')}</div><label>Audit reason<input id="discordReason" value="GM dashboard Discord setup"></label><button class="btn primary" id="saveBindings">Save Channel Bindings</button></section>
 ...
 saveBindings.onclick=()=>run('bind_channels',{announcements:bindAnnouncements.value,scenes:bindScenes.value,homes:bindHomes.value,logs:bindLogs.value,begin:bindBegin.value,info:bindInfo.value,exploration:bindExploration.value,playtest:bindPlaytest.value,updates:bindUpdates.value},'Save these Discord channel bindings?');
...
async function loadAdmin(){
 ...
 const players=d.players||[], sims=d.simulations||[], automation=d.automation||{}, lockdown=d.maintenance_mode||{}, upd=d.update||{};
 // v1.4.0: the Server Update card. The request is written by the engine and
 // run by the watcher on the NAS (update_watch.sh); this only draws the rows.
 ...
 const updStatus=updReq&&upd.in_progress?`<div><b>In progress:</b> ${esc(updReq.status)}${updReq.detail?` — ${esc(updReq.detail)}`:''} <span class="muted">(requested ${new Date(Number(updReq.requested_at)*1000).toLocaleString()})</span></div>`:'';
 ...
 app.innerHTML=`<div class="admin-hero"><div><div class="eyebrow">AUTHORITATIVE GO CONTROL PLANE</div><h2>GM Admin Console</h2><p>The levers that act on the world or the server. All mutations are executed by the Go engine and written to the canonical SQLite WAL database; dangerous actions require confirmation and are audit logged. One character's levers are on the Player Editor.</p></div><div class="tagline">${pill(pragmas.journal_mode||'DB','good')}${pill(`FK ${pragmas.foreign_keys??'?'}`,'blue')}${pill(`${players.length} players`,'warn')}</div></div>
 <div id="adminResult"></div>
 <div class="admin-grid">
  <section class="card control"><h3>👤 Player Editor</h3><p>Everything that edits one character - realm and body ladder, caps, sect, perfection, root, bloodline, physique, tribulation gates, teleport, currency, karma, fate, inventory, recovery, cooldowns, conditions, moderation, beasts, equipment and abode access - is on the Player Editor, pre-filled from the character's own rows.</p><label>Player<select id="editPlayer">${pOpts}</select></label><button class="btn primary" id="openEditor">Open the Player Editor</button></section>
  ...
  <section class="card control"><h3>🗺️ Relocate NPC</h3><p>Force-move an NPC's current location (does not affect players). Lose stages a disappearance exactly as the world's own tick does - they stay where they stand, the town learns they never came home, and the Quest Forge can draft the search; Bring back returns somebody the world lost, off-screen.</p><label>NPC<select id="npcSelect">${npcOpts}</select></label><label>New location<select id="npcLocation">${locations}</select></label><label>Reason<input id="npcReason" value="GM story move"></label><div class="buttonrow"><button class="btn" id="relocateNpc">Relocate</button><button class="btn danger" id="loseNpc">Lose</button><button class="btn" id="returnNpc">Bring back</button></div></section>
  ...
  <section class="card control${lockdown.enabled?' warnbox':''}"><h3>🔧 Maintenance mode</h3><p>Close the world while you update the server. Players are refused at every door — slash commands, hub panels and typed lines — and the scheduled world tick stands down. Administrators are unaffected, so you can always open it again.</p><div class="muted">Currently: <b>${lockdown.enabled?'CLOSED':'open'}</b>${lockdown.enabled&&lockdown.reason?` — ${esc(lockdown.reason)}`:''}</div><label>Reason players see<input id="lockdownReason" maxlength="300" placeholder="Updating to the new release, back in ten minutes" value="${esc(lockdown.reason||'')}"></label><button class="btn danger" id="lockdownOn">Close the world</button><button class="btn" id="lockdownOff">Open the world</button></section>
  <section class="card control${upd.in_progress?' warnbox':''}"><h3>⬆️ Server update</h3><p>Ask the NAS to install the newest release. The watcher beside <code>update.sh</code> closes the world, backs up the code and the database, installs, restarts and reopens the world — and rolls back if anything fails. A release that adds a <code>.env</code> key is refused and reported; that step stays yours.</p><div class="muted">${updVersions}</div><div class="muted">${updWatcher}</div>${updStatus}${updLast}<label>Reason<input id="updateReason" maxlength="300" value="GM requested update from the dashboard"></label><button class="btn danger" id="requestUpdate"${updCanAsk?'':' disabled'}>Request update</button></section>
 ...
 <h2>Recent Admin Audit</h2><button class="btn danger" id="undoLastAction">↩️ Undo most recent action</button><p class="muted">Only a specific set of simple edits can be auto-undone (karma, realm, teleport, currency grants, realm/bloodline/physique/tribulation progress, item grants, force-end-scene, NPC relocation, an NPC lost or brought back, single condition clears, and moderation). Bulk grants, Fate, sect changes, resource-cap edits, and world-time changes cannot be auto-undone.</p>${table([['ID','audit_id'],['Action','action'],['Target','target'],['Reason','reason'],['When',r=>new Date(Number(r.created_at)*1000).toLocaleString()]],d.audit||[])}`;
 ...
 openEditor.onclick=()=>openPlayerEditor(editPlayer.value);
 ...
 loseNpc.onclick=()=>run('npc.set_missing',{npc_name:npcSelect.value,missing:true,reason:npcReason.value},`Lose ${npcSelect.value} where they stand? The town learns they never came home and the Forge may draft the search.`);
 returnNpc.onclick=()=>run('npc.set_missing',{npc_name:npcSelect.value,missing:false,reason:npcReason.value},`Bring ${npcSelect.value} home off-screen?`);
 ...
 const lockReason=()=>document.getElementById('lockdownReason').value.trim();
...
// engine holds it, then every per-player lever the Admin Console used to carry,
// pre-filled from the row each one writes so the GM sees what is set before
// setting it. The Admin Console keeps the levers that act on the world or the
// server. Every action still posts to /api/admin/action and lands in
// admin_audit_log; the editor decides nothing, it only fills the form.
let EDIT_UID='';
function openPlayerEditor(uid){EDIT_UID=String(uid??'');drawer.classList.add('hidden');return switchView('player_editor')}
async function loadPlayerEditor(){
 const d=await api('/api/admin');
 if(!d.enabled){app.innerHTML=`<h2>Player Editor</h2><div class="card warnbox"><b>Admin writes are disabled.</b><div class="muted">Set DASHBOARD_ADMIN_WRITES=true and GAME_ENGINE_URL to enable authoritative controls.</div></div>`;return}
 const players=d.players||[];
 // user_id is a Discord snowflake (17-19 digits) - well past Number.MAX_SAFE_INTEGER
 // (2^53-1). It is kept as the exact string the DB returned and never wrapped in
 // Number(): rounding the low digits looks up a different, nonexistent user.
 if(!players.some(p=>sameId(p.user_id,EDIT_UID)))EDIT_UID=players.length?String(players[0].user_id):'';
 const picker=`<div id="pageActions"><select id="editorPlayer">${players.map(p=>`<option value="${esc(p.user_id)}"${sameId(p.user_id,EDIT_UID)?' selected':''}>${esc(p.name)} · ${esc(p.discord_name)}</option>`).join('')}</select></div>`;
 const el=i=>document.getElementById(i);
 const bindPicker=()=>{const sel=el('editorPlayer');if(sel)sel.onchange=()=>openPlayerEditor(sel.value)};
 if(!EDIT_UID){app.innerHTML=`${picker}${emptyState('No characters yet.','The editor opens on the first character created.')}`;bindPicker();return}
 const p=await api('/api/player?user_id='+encodeURIComponent(EDIT_UID));
 const r=p.player||{};
 if(!r.user_id){app.innerHTML=`${picker}${emptyState('Player not found.')}`;bindPicker();return}
 const locations=(d.locations||[]).map(x=>`<option${x===r.location?' selected':''}>${esc(x)}</option>`).join('');
 const currencies=(d.currencies||[]).map(x=>`<option>${esc(x)}</option>`).join('');
 const sectDatalist=(d.sects||[]).map(x=>`<option value="${esc(x)}">`).join('');
 /* The ladder and the physiques come off the content file with the row
   (v1.0.11). `gradeOpts` used to be a six-name literal here beside the
   six-name map the engine kept - two copies of one content vocabulary, free
   to disagree, and a picker built from a copy offers what the engine will
   refuse the day they do. An empty catalogue keeps whatever the character
   already has, so the card never presents an empty select as a choice. */
 const apt=d.aptitude_catalogue||{};
 const rootGrades=(apt.root_grades||[]).map(x=>x.name).filter(Boolean);
 const gradeOpts=(rootGrades.length?rootGrades:[(p.root||{}).grade].filter(Boolean)).map(x=>`<option${x===(p.root||{}).grade?' selected':''}>${esc(x)}</option>`).join('');
 const physOpts=apt.physiques||[];
 const openGate=(p.tribulations||[]).find(t=>!Number(t.cleared))||{};
 const defaultGate=Number(openGate.gate_realm_index)||[7,15,23].find(g=>g>=Number(r.realm_index||0))||23;
 ...
 const bloodlines=p.bloodlines||[], blood=bloodlines[0]||{}, phys=p.physique||{}, root=p.root||{}, abode=p.abode||{}, fate=p.fate||{}, alchemy=p.alchemy||{}, quests=p.quests||[], activeQuests=quests.filter(q=>q.status==="active");
 ...
 <h2>Player Edit</h2>
 <div class="admin-grid">
  <section class="card control"><h3>📈 Realm / Phase Override</h3><p>Directly set the cultivation realm and phase (story correction), bypassing normal breakthrough gating. The body ladder is optional: leave both fields blank to leave it unchanged, or fill both to set it in the same audited write.</p><div class="fieldrow"><label>Realm index (0-31)<input id="realmIndex" type="number" min="0" max="31" value="${esc(r.realm_index)}"></label><label>Phase (1-9)<input id="realmPhase" type="number" min="1" max="9" value="${esc(r.phase)}"></label></div><div class="fieldrow"><label>Body realm index (0-31)<input id="realmBodyIndex" type="number" min="0" max="31" value="${esc(r.body_realm_index)}"></label><label>Body phase (1-9)<input id="realmBodyPhase" type="number" min="1" max="9" value="${esc(r.body_phase)}"></label></div>${reasonField('realmReason','GM story correction')}<button class="btn primary" id="setRealm">Set realm</button></section>
  <section class="card control"><h3>🧘 Realm Perfection Progress</h3><p>Directly set cultivation or body-realm perfection progress for one realm. The rows below are what the engine holds now.</p>${table([['Track','track'],['Realm','realm_index'],['Active',x=>x.active?'Yes':'No'],['Done',x=>x.completed?'Yes':'No'],['Progress',x=>`${x.progress}%`],['Quests','completed_quests']],p.perfection||[],{empty:'No perfection path started.'})}<div class="fieldrow"><label>Track<select id="perfTrack"><option value="cultivation">Cultivation</option><option value="body">Body</option></select></label><label>Realm index (0-31)<input id="perfRealmIndex" type="number" min="0" max="31" value="${esc(r.realm_index)}"></label><label>Progress (0-100)<input id="perfProgress" type="number" min="0" max="100" value="0"></label></div>${reasonField('perfReason','GM story correction')}<button class="btn primary" id="setRealmPerfection">Set perfection progress</button></section>
  <section class="card control"><h3>🌀 Spiritual Root</h3><p>Edit grade, purity and mutation. Elements, stability (${esc(root.stability??'—')}%), refinement (${esc(root.refinement_progress??'—')}%) and compatibility are left untouched.</p><div class="fieldrow"><label>Grade<select id="rootGrade">${gradeOpts}</select></label><label>Purity (0-100)<input id="rootPurity" type="number" min="0" max="100" value="${esc(root.purity??50)}"></label></div><label>Mutation (optional)<input id="rootMutation" value="${esc(root.mutation||'')}" placeholder="e.g. Chaos-Attuned"></label>${reasonField('rootReason','GM story correction')}<button class="btn primary" id="setSpiritualRoot">Set spiritual root</button></section>
  <section class="card control"><h3>🩸 Bloodline</h3>${bloodlines.length?`<p>Edit purity, evolution stage and progress on a bloodline this character carries. Name, affinity and state (${esc(blood.state)}) are left unchanged.</p><label>Bloodline<select id="bloodId">${bloodlines.map(b=>`<option value="${esc(b.bloodline_id)}">${esc(b.name)} (${esc(b.bloodline_id)})${Number(b.primary_lineage)?' · primary':''}</option>`).join('')}</select></label><div class="fieldrow"><label>Purity (0-100)<input id="bloodPurity" type="number" min="0" max="100" value="${esc(blood.purity??0)}"></label><label>Evolution stage<input id="bloodEvolution" type="number" min="0" value="${esc(blood.evolution_stage??0)}"></label><label>Progress (0-100)<input id="bloodProgress" type="number" min="0" max="100" value="${esc(blood.progress??0)}"></label></div>${reasonField('bloodReason','GM story correction')}<button class="btn primary" id="setBloodline">Set bloodline</button>`:`<p>This character was born without a bloodline. The lever edits a bloodline row that exists; it cannot grant one.</p>`}</section>
  <section class="card control"><h3>🫀 Physique</h3>${phys.physique_id?`<p>${esc(phys.name||phys.physique_id)} · ${esc(phys.state)} · instability ${esc(phys.instability??0)}. Edit evolution, progress and stability.</p><div class="fieldrow"><label>Physique${physOpts.length?`<select id="physId"><option value="">— leave as ${esc(phys.name||phys.physique_id)} —</option>${physOpts.map(x=>`<option value="${esc(x.id)}"${sameId(x.id,phys.physique_id)?' selected':''}>${esc(x.name)}</option>`).join('')}</select>`:`<span class="mut">catalogue unavailable; the numbers below still edit</span>`}</label><label>Evolution stage<input id="physEvolution" type="number" min="0" value="${esc(phys.evolution_stage??0)}"></label><label>Progress (0-100)<input id="physProgress" type="number" min="0" max="100" value="${esc(phys.progress??0)}"></label><label>Stability (0-100)<input id="physStability" type="number" min="0" max="100" value="${esc(phys.stability??100)}"></label></div>${reasonField('physReason','GM story correction')}<button class="btn primary" id="setPhysique">Set physique</button>`:`<p>No physique row exists for this character yet; the lever edits a row that exists.</p>`}</section>
  <section class="card control dangerzone"><h3>⚡ Tribulation Gate</h3><p>Clear unlocks the breakthrough past this gate. Reset wipes preparation, attempts and cleared for a clean re-attempt.</p>${table([['Gate','gate_realm_index'],['Preparation','preparation'],['Attempts','attempts'],['Cleared',x=>pill(Number(x.cleared)?'CLEARED':'OPEN',Number(x.cleared)?'good':'warn')],['Last','last_result']],p.tribulations||[],{empty:'No tribulation gate touched yet.'})}<label>Gate<select id="tribGate">${gateOpts}</select></label>${reasonField('tribReason','GM unstuck the player')}<div class="buttonrow"><button class="btn danger" id="clearTribulation">Clear gate</button><button class="btn danger" id="resetTribulation">Reset gate</button></div></section>
  <section class="card control"><h3>🧭 Teleport</h3><p>Standing at ${esc(r.location)}. Teleporting resets the scene to the destination.</p><label>Canonical location<select id="teleLocation">${locations}</select></label>${reasonField('teleReason','GM teleport')}<button class="btn primary" id="teleportPlayer">Teleport</button></section>
  <section class="card control"><h3>💰 Grant Currency</h3>${table([['Currency','currency_id'],['Balance',x=>n(x.balance)]],p.wallets||[],{empty:'No wallet yet.'})}<div class="fieldrow"><label>Currency<select id="currency">${currencies}</select></label><label>Amount<input id="currencyAmount" type="number" min="1" value="100"></label></div>${reasonField('currencyReason','GM reward')}<button class="btn primary" id="grantCurrency">Grant</button></section>
  <section class="card control"><h3>☯ Karma &amp; Fate</h3><p>Karma ${esc(r.karma_score)} · Fate ${esc(fate.points??0)} (earned ${esc(fate.lifetime_earned??0)}, spent ${esc(fate.lifetime_spent??0)}). Both levers apply a delta.</p><div class="fieldrow"><label>Karma delta<input id="karmaDelta" type="number" value="10"></label><label>Fate delta (-9..9)<input id="fateDelta" type="number" value="1" min="-9" max="9"></label></div>${reasonField('karmaReason','GM correction')}<div class="buttonrow"><button class="btn" id="adjustKarma">Adjust karma</button><button class="btn" id="adjustFate">Adjust fate</button></div></section>
  <section class="card control"><h3>📜 Quests</h3><p>What this player holds. A report replays one objective the player already did - an objective is counted only while its quest is active, so one done too early is lost. Completing pays the quest's reward and hands over its next stage, exactly as a player's last report would; neither can be undone.</p>${table([['Quest',x=>esc(x.title)],['Status','status'],['Objectives',x=>x.objectives.map(o=>`${esc(o.label)} ${esc(o.progress)}/${esc(o.count)}`).join('<br>')]],quests,{empty:'No quests held.'})}${activeQuests.length?`<div class="fieldrow"><label>Quest<select id="questKey">${activeQuests.map(q=>`<option value="${esc(q.quest_key)}">${esc(q.title)}</option>`).join('')}</select></label><label>Objective<select id="questObjective"></select></label><label>Amount<input id="questAmount" type="number" value="1" min="1" max="1000"></label></div>${reasonField('questReason','Missed objective report')}<div class="buttonrow"><button class="btn" id="reportQuest">Report objective</button><button class="btn danger" id="completeQuest">Complete quest</button></div>`:'<p class="muted">No active quest to advance.</p>'}</section>
  <section class="card control"><h3>❤️‍🩹 Resource Caps</h3><p>Vitality ${esc(r.vitality)}/${esc(r.vitality_max)} · Qi ${esc(r.qi)}/${esc(r.qi_max)}. Leave a field blank to leave it unchanged; lowering a cap clamps the current value down to match.</p><div class="fieldrow"><label>Vitality max<input id="capsVitality" type="number" min="1" placeholder="${esc(r.vitality_max)} (unchanged)"></label><label>Qi max<input id="capsQi" type="number" min="1" placeholder="${esc(r.qi_max)} (unchanged)"></label></div>${reasonField('capsReason','GM correction')}<button class="btn primary" id="setResourceCaps">Set caps</button></section>
  <section class="card control"><h3>⛩️ Sect Membership</h3><p>${r.sect_name?`${esc(r.sect_name)} · ${esc(r.rank_name)} (rank ${esc(r.rank_level??0)})`:'Independent'}. Assign, re-rank, or remove the membership.</p><label>Sect name<input id="sectName" list="sectList" value="${esc(r.sect_name||'')}" placeholder="e.g. Azure Cloud Sect"></label><datalist id="sectList">${sectDatalist}</datalist><div class="fieldrow"><label>Rank name<input id="sectRankName" value="${esc(r.rank_name||'Disciple')}"></label><label>Rank level (0-100)<input id="sectRankLevel" type="number" min="0" max="100" value="${esc(r.rank_level??0)}"></label></div>${reasonField('sectReason','GM story correction')}<div class="buttonrow"><button class="btn primary" id="setSect">Set sect / rank</button><button class="btn danger" id="removeSect">Remove sect membership</button></div></section>
  <section class="card control"><h3>🎒 Inventory</h3><p>Grant (positive) or remove (negative) an item by quantity. Click a row to fill the item.</p>${table([['Item',x=>`<span class="pick" data-pick-item="${esc(x.item_id)}">${esc(x.item_id)}</span>`],['Qty','quantity']],p.inventory||[],{empty:'Carrying nothing.'})}<div class="fieldrow"><label>Item ID<input id="itemId" placeholder="e.g. spirit_herb (or the item name)"></label><label>Quantity delta<input id="itemDelta" type="number" value="1"></label></div>${reasonField('itemReason','GM item grant')}<button class="btn primary" id="adjustItem">Adjust item</button></section>
  <section class="card control dangerzone"><h3>❤️ Recovery</h3><p>Emergency character-state recovery. Force reincarnation ready clears the real-world-time wait on a soul in the afterlife so they can reincarnate immediately, without affecting anything else about their Samsara cycle.</p>${reasonField('recoverReason','GM recovery')}<div class="buttonrow"><button class="btn danger" id="revivePlayer">Revive</button><button class="btn danger" id="clearBattle">Clear battle</button><button class="btn danger" id="forceEndScene">Force end scene</button><button class="btn danger" id="forceReincarnationReady">Force reincarnation ready</button></div><h4>Cooldowns</h4>${table([['Action',x=>`<span class="pick" data-pick-cooldown="${esc(x.action)}">${esc(x.action)}</span>`],['Available at',x=>new Date(Number(x.available_at)*1000).toLocaleString()]],p.cooldowns||[],{empty:'Nothing on cooldown.'})}<label>Cooldown to clear (blank = all; click a row to fill)<input id="cooldownAction" placeholder="e.g. explore"></label><button class="btn danger" id="resetCooldowns">Reset cooldowns</button></section>
  <section class="card control"><h3>✚ Conditions</h3><p>Active conditions, by severity. Clearing one is audited and can be undone.</p>${table([['Condition','name'],['Category','category'],['Severity','severity'],['',c=>`<button class="btn" data-clear-condition="${esc(c.condition_id)}">Clear</button>`]],p.conditions||[],{empty:'No active conditions.'})}</section>
  <section class="card control dangerzone"><h3>🚫 Moderation</h3><p>Freeze blocks every authoritative action this player takes. Mute blocks only free-form roleplay (scene actions) - combat, cultivation, shopping and other checks stay open. Ban blocks everything and never expires. A mute or freeze given a duration lifts itself when it runs out; leave it at 0 to hold until lifted. The same verbs exist as <code>/admin player mute|freeze|ban</code> in Discord and write the same audit row. Scope: only actions routed through the Go authoritative engine, so treat it as a moderation nudge, not anti-cheat.</p><div class="fieldrow"><label class="toggle"><span>Muted</span><input type="checkbox" id="modMuted"${Number(r.is_muted)?' checked':''}></label><label class="toggle"><span>Frozen</span><input type="checkbox" id="modFrozen"${Number(r.is_frozen)?' checked':''}></label><label class="toggle"><span>Banned</span><input type="checkbox" id="modBanned"${Number(r.is_banned)?' checked':''}></label></div><label>Expires after (hours, 0 = until lifted; applies to the mute and freeze set now)<input id="modHours" type="number" min="0" step="0.5" value="0"></label><p class="muted" id="modStanding"></p><label>Reason (shown to the player)<input id="modReason" value="${esc(r.moderation_reason||'')}" placeholder="e.g. cooling off period"></label><button class="btn danger" id="setModeration">Apply moderation</button></section>
  <section class="card control dangerzone"><h3>🧪 Pill Toxicity</h3><p>Now ${esc(alchemy.pill_toxicity??0)}. Set it directly (0-1000).</p><label>Pill toxicity<input id="toxicityValue" type="number" min="0" max="1000" value="${esc(alchemy.pill_toxicity??0)}"></label>${reasonField('craftReason','GM crafting fix')}<button class="btn danger" id="setPillToxicity">Set toxicity</button></section>
  <section class="card control dangerzone"><h3>🐾 Spirit Beasts</h3>${(p.beasts||[]).length?`${table([['ID','beast_id'],['Name','name'],['Species','species'],['Loyalty','loyalty'],['Evolution','evolution_stage'],['Active',x=>Number(x.active)?'Yes':'']],p.beasts)}<label>Beast<select id="beastId">${(p.beasts||[]).map(b=>`<option value="${esc(b.beast_id)}" data-loyalty="${esc(b.loyalty)}" data-evolution="${esc(b.evolution_stage)}">${esc(b.name)} (${esc(b.species)})</option>`).join('')}</select></label><div class="fieldrow"><label>Loyalty (0-100, blank = unchanged)<input id="beastLoyalty" type="number" min="0" max="100" placeholder="unchanged"></label><label>Evolution stage (blank = unchanged)<input id="beastEvolution" type="number" min="0" placeholder="unchanged"></label></div>${reasonField('beastReason','GM crafting fix')}<button class="btn danger" id="setBeastStats">Set beast stats</button>`:'<p>No tamed beast.</p>'}</section>
  <section class="card control dangerzone"><h3>🗡️ Equipment</h3><p>Force-remove a bound piece. This cannot be undone through normal gameplay.</p>${table([['ID','equipment_id'],['Item','item_id'],['Slot','slot'],['Durability',x=>`${x.durability}/${x.max_durability}`],['Equipped',x=>Number(x.equipped)?'Yes':''],['',x=>`<button class="btn danger" data-remove-equipment="${esc(x.equipment_id)}">Remove</button>`]],p.equipment||[],{empty:'Nothing bound.'})}${reasonField('equipReason','GM crafting fix')}</section>
  <section class="card control"><h3>🏠 Abode Access</h3>${abode.location_key?`<p>${esc(abode.name)} (${esc(abode.grade)}) at ${esc(abode.base_location)}.</p>${table([['Guest',x=>esc(x.name||x.guest_user_id)],['Role','access_role'],['',x=>`<button class="btn danger" data-revoke-guest="${esc(x.guest_user_id)}">Revoke</button>`]],p.guests||[],{empty:'No guest has access.'})}<div class="fieldrow"><label>Guest<select id="abodeGuest">${guestOpts}</select></label><label>Access role<input id="abodeAccessRole" value="guest"></label></div>${reasonField('abodeReason','GM crafting fix')}<button class="btn" id="grantAbodeAccess">Grant access</button>`:'<p>This character owns no property.</p>'}</section>
 </div>`;
 bindPicker();
 const out=el('adminResult');
 const run=async(action,payload,confirmText='')=>{if(confirmText&&!confirm(confirmText))return;out.innerHTML='<div class="loading">Applying authoritative action…</div>';try{const res=await adminPost(action,payload);out.innerHTML=resultBox(res.result??res);setTimeout(()=>refresh().catch(()=>{}),900)}catch(e){out.innerHTML=resultBox(e.message,false)}};
 const uid=EDIT_UID, reason=i=>(el(i)||{}).value||'GM dashboard';
 const on=(i,fn)=>{const node=el(i);if(node)node.onclick=fn};
 on('setRealm',()=>{const payload={user_id:uid,realm_index:Number(realmIndex.value),phase:Number(realmPhase.value),reason:realmReason.value};const bodyIndex=realmBodyIndex.value,bodyPhase=realmBodyPhase.value;if((bodyIndex==='')!==(bodyPhase==='')){out.innerHTML=resultBox('Body realm index and body phase are set together: fill both or leave both blank.',false);return}let confirmText=`Set this player's realm to ${realmIndex.value}/${realmPhase.value}`;if(bodyIndex!==''){payload.body_realm_index=Number(bodyIndex);payload.body_phase=Number(bodyPhase);confirmText+=` and their body realm to ${bodyIndex}/${bodyPhase}`}run('player.set_realm',payload,confirmText+', bypassing breakthrough gating?')});
 on('setRealmPerfection',()=>run('player.set_realm_perfection',{user_id:uid,track:perfTrack.value,realm_index:Number(perfRealmIndex.value),progress:Number(perfProgress.value),reason:reason('perfReason')}));
 on('setSpiritualRoot',()=>run('player.set_spiritual_root',{user_id:uid,grade:rootGrade.value,purity:Number(rootPurity.value),mutation:rootMutation.value,reason:reason('rootReason')}));
 const bloodSel=el('bloodId');
 if(bloodSel){const fill=()=>{const b=bloodlines.find(x=>x.bloodline_id===bloodSel.value)||{};bloodPurity.value=b.purity??0;bloodEvolution.value=b.evolution_stage??0;bloodProgress.value=b.progress??0};bloodSel.onchange=fill;
  on('setBloodline',()=>run('player.set_bloodline',{user_id:uid,bloodline_id:bloodSel.value,purity:Number(bloodPurity.value),evolution_stage:Number(bloodEvolution.value),progress:Number(bloodProgress.value),reason:reason('bloodReason')}))}
 /* `physique_id` is sent only when the GM picked a different one, so every
   existing use of this card still edits the three numbers and nothing else -
   which is the same shape the realm card uses for its optional body pair. */
 on('setPhysique',()=>{const picked=(typeof physId!=='undefined'&&physId)?String(physId.value||''):'';const payload={user_id:uid,evolution_stage:Number(physEvolution.value),progress:Number(physProgress.value),stability:Number(physStability.value),reason:reason('physReason')};if(picked&&!sameId(picked,phys.physique_id))payload.physique_id=picked;return run('player.set_physique',payload)});
 on('clearTribulation',()=>run('player.set_tribulation',{user_id:uid,gate_realm_index:Number(tribGate.value),mode:'clear',reason:reason('tribReason')},'Clear this tribulation gate, unlocking the breakthrough past it?'));
 on('resetTribulation',()=>run('player.set_tribulation',{user_id:uid,gate_realm_index:Number(tribGate.value),mode:'reset',reason:reason('tribReason')},'Reset this tribulation gate (wipe preparation/attempts/cleared) for a clean re-attempt?'));
 on('teleportPlayer',()=>run('player.teleport',{user_id:uid,location:teleLocation.value,reason:reason('teleReason')},'Teleport this player and reset their scene location?'));
 on('grantCurrency',()=>run('player.grant_currency',{user_id:uid,currency_id:currency.value,amount:Number(currencyAmount.value),reason:reason('currencyReason')}));
 on('adjustKarma',()=>run('player.karma',{user_id:uid,delta:Number(karmaDelta.value),reason:reason('karmaReason')}));
 on('adjustFate',()=>run('player.fate',{user_id:uid,delta:Number(fateDelta.value),reason:reason('karmaReason')}));
 const questSel=el('questKey');
 if(questSel){const fillObjectives=()=>{const q=activeQuests.find(x=>x.quest_key===questSel.value)||{objectives:[]};const types=[...new Set(q.objectives.filter(o=>o.progress<o.count).map(o=>o.type))];questObjective.innerHTML=types.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('')};questSel.onchange=fillObjectives;fillObjectives();
  on('reportQuest',()=>run('player.quest_progress',{user_id:uid,quest_key:questSel.value,objective_type:questObjective.value,amount:Number(questAmount.value),reason:reason('questReason')}));
  on('completeQuest',()=>run('player.quest_complete',{user_id:uid,quest_key:questSel.value,reason:reason('questReason')},'Complete this quest, pay its reward and hand over its next stage? This cannot be undone.'))}
 on('setResourceCaps',()=>{const payload={user_id:uid,reason:reason('capsReason')};if(capsVitality.value!=='')payload.vitality_max=Number(capsVitality.value);if(capsQi.value!=='')payload.qi_max=Number(capsQi.value);if(payload.vitality_max===undefined&&payload.qi_max===undefined){out.innerHTML=resultBox('Set at least one of vitality_max or qi_max.',false);return}run('player.set_resource_caps',payload)});
 on('setSect',()=>run('player.set_sect',{user_id:uid,sect_name:sectName.value,rank_name:sectRankName.value,rank_level:Number(sectRankLevel.value),reason:reason('sectReason')}));
 on('removeSect',()=>run('player.set_sect',{user_id:uid,remove:true,reason:reason('sectReason')},'Remove this player\'s sect membership entirely?'));
 document.querySelectorAll('[data-pick-item]').forEach(node=>node.onclick=()=>{itemId.value=node.dataset.pickItem});
 on('adjustItem',()=>run('player.adjust_item',{user_id:uid,item_id:itemId.value,quantity:Number(itemDelta.value),reason:reason('itemReason')}));
 on('revivePlayer',()=>run('player.revive',{user_id:uid,reason:reason('recoverReason')},'Revive this character, refill Vitality/Qi, cancel Samsara, and clear active battles?'));
 on('clearBattle',()=>run('player.clear_battle',{user_id:uid,reason:reason('recoverReason')},'Force-abandon all active battles for this player, including group combat and PvP?'));
 on('forceEndScene',()=>run('player.force_end_scene',{user_id:uid,reason:reason('recoverReason')},'Reset this player to a world scene at their current location, without teleporting them?'));
 on('forceReincarnationReady',()=>run('player.force_reincarnation_ready',{user_id:uid,reason:reason('recoverReason')},'Clear the real-world-time wait so this soul can reincarnate immediately? Only affects a soul currently waiting in the afterlife.'));
 document.querySelectorAll('[data-pick-cooldown]').forEach(node=>node.onclick=()=>{cooldownAction.value=node.dataset.pickCooldown});
 on('resetCooldowns',()=>run('player.reset_cooldowns',{user_id:uid,action:cooldownAction.value,reason:reason('recoverReason')},cooldownAction.value?`Clear the "${cooldownAction.value}" cooldown for this player?`:'Clear ALL cooldowns for this player?'));
 document.querySelectorAll('[data-clear-condition]').forEach(node=>node.onclick=()=>run('player.clear_condition',{user_id:uid,condition_id:Number(node.dataset.clearCondition),reason:'GM cleared condition'},'Clear this condition?'));
 const untilText=(until)=>until>0?(until*1000<=Date.now()?' (lapsed, clears on the next tick)':` until ${new Date(until*1000).toLocaleString()}`):' until lifted';
 const held=[Number(r.is_banned)?'banned':'',Number(r.is_frozen)?'frozen'+untilText(Number(r.frozen_until||0)):'',Number(r.is_muted)?'muted'+untilText(Number(r.muted_until||0)):''].filter(Boolean);
 if(el('modStanding'))el('modStanding').textContent=held.length?'Standing: '+held.join(' · '):'Standing: nothing in force';
 on('setModeration',()=>{const seconds=Math.max(0,Math.round(Number(modHours.value||0)*3600));const payload={user_id:uid,muted:modMuted.checked,frozen:modFrozen.checked,banned:modBanned.checked,moderation_reason:modReason.value,reason:modReason.value||'GM moderation'};if(seconds>0){payload.muted_seconds=seconds;payload.frozen_seconds=seconds}run('player.set_moderation',payload,`Set this player to ${modMuted.checked?'muted':'not muted'}, ${modFrozen.checked?'frozen':'not frozen'} and ${modBanned.checked?'banned':'not banned'}${seconds>0?` for ${modHours.value} hour(s)`:''}?`)});
 on('setPillToxicity',()=>run('player.set_pill_toxicity',{user_id:uid,pill_toxicity:Number(toxicityValue.value),reason:reason('craftReason')},`Set pill toxicity to ${toxicityValue.value}?`));
 on('setBeastStats',()=>{const payload={user_id:uid,beast_id:Number(beastId.value),reason:reason('beastReason')};if(beastLoyalty.value!=='')payload.loyalty=Number(beastLoyalty.value);if(beastEvolution.value!=='')payload.evolution_stage=Number(beastEvolution.value);run('player.set_beast_stats',payload,`Update spirit beast ${beastId.value}?`)});
 document.querySelectorAll('[data-remove-equipment]').forEach(node=>node.onclick=()=>run('player.remove_equipment',{user_id:uid,equipment_id:Number(node.dataset.removeEquipment),reason:reason('equipReason')},`Force-remove equipment ${node.dataset.removeEquipment}? This cannot be undone through normal gameplay.`));
 on('grantAbodeAccess',()=>run('player.set_abode_access',{owner_user_id:uid,guest_user_id:abodeGuest.value,access_role:abodeAccessRole.value,reason:reason('abodeReason')},'Grant abode access to this guest?'));
 document.querySelectorAll('[data-revoke-guest]').forEach(node=>node.onclick=()=>run('player.set_abode_access',{owner_user_id:uid,guest_user_id:node.dataset.revokeGuest,revoke:true,reason:reason('abodeReason')},'Revoke this guest\'s abode access?'));
}
...
const loaders={overview:loadOverview,timeline:loadTimeline,npcs:loadNPCs,npc_population:loadNpcPopulation,npc_families:loadNpcFamilies,npc_society:loadNpcSociety,npc_deeds:loadNpcDeeds,families:loadFamilies,sects:loadSects,sect_members:loadSectMembers,sect_recruitment:loadSectRecruitment,sect_holdings:loadSectHoldings,conflicts:loadConflicts,events:loadEvents,players:loadPlayers,cultivation:loadCultivation,crafting:loadCrafting,exploration:loadExploration,commissions:loadCommissions,quests:loadQuests,economy:loadEconomy,dynasties:loadDynasties,party:loadParty,pvp:loadPvp,conditions:loadConditions,threads:loadThreads,rag:loadRag,decisions:loadDecisions,discord:loadDiscordSetup,ai_routing:loadAiRouting,narration:loadNarration,admin:loadAdmin,player_editor:loadPlayerEditor};
