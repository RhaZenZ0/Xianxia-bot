...
async function loadPlayerEditor(){
 ...
 const gradeOpts=['Mortal','Common','Refined','Earth','Heaven','Immortal'].map(x=>`<option${x===(p.root||{}).grade?' selected':''}>${esc(x)}</option>`).join('');
 ...
 on('setPhysique',()=>run('player.set_physique',{user_id:uid,evolution_stage:Number(physEvolution.value),progress:Number(physProgress.value),stability:Number(physStability.value),reason:reason('physReason')}));
