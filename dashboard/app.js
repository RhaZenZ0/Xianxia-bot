...
async function loadDiscordSetup(){
 ...
 <div class="grid2">
  ...
  <section><h2>Auction Houses</h2><p class="muted">One live channel per auction house (v0.33.1), beside the capitals: a lot is posted the moment it is listed, its card follows every bid, and it is struck when the simulation tick settles it. Visible to cultivators who can reach that house's world. Setup/Repair creates the channels.</p>${table([['House','name'],['World','world'],['Entrance','entrance'],['State',r=>pill(r.ready?'ready':'missing',r.ready?'good':'warn')],['Channel',r=>r.channel_name?`#${esc(r.channel_name)}`:'—'],['Open lots',r=>n(r.open_lots)]],halls)}</section>
 </div>
